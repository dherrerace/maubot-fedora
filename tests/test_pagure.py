...
async def test_pagureissue(bot, plugin, respx_mock, command, project):
    ...
    two_weeks_ago_ts = int((datetime.datetime.now(datetime.UTC) + datetime.timedelta(weeks=-2)).timestamp())
    one_week_ago_ts = int((datetime.datetime.now(datetime.UTC) + datetime.timedelta(weeks=-1)).timestamp())
    response = {
        "assignee": None,
        "blocks": [],
        "close_status": None,
